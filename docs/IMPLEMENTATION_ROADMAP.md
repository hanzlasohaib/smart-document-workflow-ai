...
- [ ] Protected actions redirect to Login with return URL
- [ ] `user` / `admin` land on correct dashboards; no admin signup
- [ ] Staging E2E happy path: register/login → upload → process → review/approve → notification
- [ ] Public pages at MVP thin quality
...
| Queue-backed worker (same codebase) | Only if load/isolation/retry triggers met |
| Backup/restore runbook (Postgres + object storage) | Always; test once on staging |
| Expand tests (FE smoke, more pipeline, migration dry-run in CI) | Always |

### P3 Definition of Done

- [ ] Staging + prod: health/readiness, structured logs, error tracking
- [ ] Auth rate-limited; CORS locked to known FE origins
- [ ] List endpoints paginated
- [ ] Backup/restore documented and tested on staging
- [ ] Queue adopted under ADR-02-004 **or** explicitly deferred with measured rationale
